...
func disparar():
	if balas_cargador > 0 and puede_disparar:
		$Timer.start()
		$Sprites.play("disparo")
		$Sonido_disparo.play()
		var aux = BALA.instantiate()
		aux.get_node("Texturas").scale = Vector2(0.4, 0.4) 
		aux.get_node("Texturas").play("mp40")
	...
	elif balas_cargador == 0:
		$Sonido_cargador_vacio.play()
		
func recargar():
	if cargadores > 0:
		if balas_cargador < 32:
			$Sonido_recarga.play()
