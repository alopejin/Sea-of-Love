...
@onready var zombies = preload("res://juego3/escenas/zombie_2.tscn")
...
func generar_zombies_izd():
	for i in range(zombies_izquierda):
		...
		add_child(aux)
		aux.global_position = Vector2i(randi_range(-500, -40), 491)
...
func generar_zombies_der():
	for i in range(zombies_derecha):
		var aux = zombies.instantiate()
		add_child(aux)
		aux.global_position = Vector2i(randi_range(1192, 1652), 491)
		aux.mod_velocidad = randi_range(0, 30)
